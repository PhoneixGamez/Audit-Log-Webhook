    ...
    try{
      ...
      c.on("guildMemberUpdate", function (oldMember, newMember) {
          ...
          if (rolechanged) {
              ...
              let text = `${roleremoved ? `<a:896694904019894282:901591282005200946> ROLE REMOVED: \n${roleremoved}` : ""}${roleadded ? `<a:892035791449100348:901591311528906812> ROLE ADDED:\n${roleadded}` : ""}`
              send_log(c,
                oldMember.guild,
                `${roleadded ? "GREEN" : "RED"}`,
...
async function send_log(c, guild, color, title, description, thumb){
  try {
    ...
    .setFooter(guild.name + " | Made by The Pheonix#7644!", guild.iconURL({format: "png"}))
    //GET THE CHANNEL
